...
User Function: DSOAPF01 - Fun��o para Integra��o Via SOAP com o TOTVS Corpore RM
@OWNER PanCristal
@VERSION PROTHEUS 12
@SINCE 10/07/2024
@Permite
...
Realiza a consulta de clientes atrav�s da API padr�o RealizarConsultaSQL no RM (Completo)
/*/
//------------------------------------------------------------------------------------------
            ...
            If !oXML:Parse( cResult )
                ...
                For nY := 1 To oXML:XPathChildCount('/ns:Envelope/ns:Body/ns1:RealizarConsultaSQLResponse/ns1:RealizarConsultaSQLResult/ns1:NewDataSet')
                    ...
                    If !Empty(aRegXML)
                        ...
                        oSA1Mod:setValue("A1_END"    ,aRegXML[08][03] + ", " + aRegXML[09][03]      ) // Endereco + N�mero
                        ...
                        If !Empty(Upper(aRegXML[52][03])) .And. SYA->(MSSeek(xFilial("SYA")+Upper(aRegXML[52][03])))
                            oSA1Mod:LoadValue("A1_PAIS"   ,SYA->YA_CODGI                            ) // Codigo do Pa�s
                        EndIF
                        If !Empty(Upper(aRegXML[52][03])) .And. CCH->(MSSeek(xFilial("CCH")+Upper(aRegXML[52][03])))
                            oSA1Mod:LoadValue("A1_CODPAIS",Alltrim(CCH->CCH_CODIGO)                 ) // Codigo do Pa�s Bacen.
                        EndIF
...
Realiza a consulta de clientes atrav�s da API padr�o RealizarConsultaSQL no RM (Resumido)
/*/
//------------------------------------------------------------------------------------------
            ...
            If !oXML:Parse( cResult )
                ...
                For nY := 1 To oXML:XPathChildCount('/ns:Envelope/ns:Body/ns1:RealizarConsultaSQLResponse/ns1:RealizarConsultaSQLResult/ns1:NewDataSet')
                    ...
                    If !Empty(aRegXML)
                        ...
                        oSA1Mod:setValue("A1_END"    ,Upper(Pad(aRegXML[08][03] + ", " + aRegXML[09][03] ,FWTamSX3("A1_END")[1]))   ) // Endereco + N�mero
...
Realiza a consulta de Produtos atrav�s da API padr�o RealizarConsultaSQL no RM
/*/
//------------------------------------------------------------------------------------------
...
Realiza a consulta da Tabela de Pre�o atrav�s da API padr�o RealizarConsultaSQL no RM
/*/
//------------------------------------------------------------------------------------------
                ...
                For nY := 1 To oXML:XPathChildCount('/ns:Envelope/ns:Body/ns1:RealizarConsultaSQLResponse/ns1:RealizarConsultaSQLResult/ns1:NewDataSet')
                    ...
                    If !Empty(aRegXML)
                        ...
                        If lMsErroAuto
                            ...
                            fnGrvLog(cEndPoint,cBody,'',cErro,"Erro Tabela de Pre�o: " + StrZero(Val(aRegXML[02][03]), FWTamSX3("DA0_CODTAB")[1]) + " - " +aRegXML[07][03],cValToChar(nOpc),"Integracao Tabela de Pre�o")
                        Else
                            fnGrvLog(cEndPoint,cBody,'',,"Tabela de Pre�o: " + StrZero(Val(aRegXML[02][03]), FWTamSX3("DA0_CODTAB")[1]) + " - " +aRegXML[07][03],cValToChar(nOpc),"Integracao Tabela de Pre�o")
                        EndIf
...
Realiza a consulta de estoque atrav�s da API padr�o employeeDataContent no RM
/*/
//-----------------------------------------------------------------------------
...
Realiza a consulta de Ponto de Venda atrav�s da API padr�o RealizarConsultaSQL no RM
/*/
//------------------------------------------------------------------------------------------
...
Realiza a consulta do C�digo de Barras atrav�s da API padr�o RealizarConsultaSQL no RM
/*/
//------------------------------------------------------------------------------------------
...
Realiza a consulta do Vendedor/Operador atrav�s da API padr�o RealizarConsultaSQL no RM
/*/
//------------------------------------------------------------------------------------------
...
Realiza o envio da Nota Fiscal de Sa�da para o Copore RM
/*/
//-----------------------------------------------------------------------------
...
Static Function fEnvNFeVend()
    ...
    cBody += '                                  <CODLOC>' + cLocEstoq + '</CODLOC> ' //C�digo do Local de Destino
    cBody += '                                  <CODCFO>' + SF1->F1_FORNECE + '</CODCFO> ' //C�digo do Cliente / Fornecedor
    ...
    cBody += '                                  <CODCCUSTO></CODCCUSTO> ' //N�o ter� Centro de Custo no cabe�alho
    ...
    While !SL2->(Eof()) .AND. ( SL2->L2_FILIAL  == SL1->L1_FILIAL ); 
        ...
        cBody += '                                  <CODCCUSTO></CODCCUSTO> ' //Rateio de Centro de Custo do Item n�o ter�
        cBody += '                                  <VALOR></VALOR> '
        cBody += '                                  <IDMOVRATCCU>-1</IDMOVRATCCU> '
        cBody += '                              </TITMMOVRATCCU> '
        cBody += '                              <TMOVCOMPL> '
...
Static Function fEnvNFeDev()
    ...
    cBody += '                                  <PERCCOMISSAOVEN4>0.0000</PERCCOMISSAOVEN4> '
    cBody += '                                  <STATUSMOVINCLUSAOCOLAB>0</STATUSMOVINCLUSAOCOLAB> '
    cBody += '                                  <IDMOVRELAC>' + " " + '</IDMOVRELAC> ' //Verificar o ID Relacionado da NF de Sa�da
    cBody += '                              </TMOV> '
    cBody += '                              <TNFE> '
    cBody += '                                  <CODCOLIGADA>' + cCodEmp + '</CODCOLIGADA> '
    cBody += '                                  <IDMOV>-1</IDMOV> '
    ...
    cBody += '                              </TMOVPAGTO> '
    //Itens da Nota Fiscal de Entrada (Devolu��o)
    DBSelectArea("SD1")
...
Static Function fCanFinan()
    ...
    cBody := ' 							<FailureMessage xmlns="http://www.totvs.com/">Falha na execu��o do processo</FailureMessage> '
...
Grava o LOG de integra��o na tabela SZ1 - Log de Integra��o Protheus x RM
/*/
//-----------------------------------------------------------------------------
